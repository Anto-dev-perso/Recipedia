...
describe('SettingsItemList Component', () => {
    ...
    const mockIngredients = [
        ingredientsDataset[1],
        ingredientsDataset[4],
    ];
    ...
    describe('ingredient', () => {
        ...
        test('renders', () => {
            const {getByTestId} = render(
                <SettingsItemList {...defaultProps}            />
            );

            expect(getByTestId(`${defaultProps.testIdPrefix}::Title`).props.children).toEqual('ingredients');

            let index = 0;
            for (const ingredient of defaultProps.items) {
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::Type`).props.children).toEqual('ingredient');
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::Item`).props.children).toEqual(JSON.stringify(ingredient));
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::OnEdit`)).toBeTruthy();
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::OnDelete`)).toBeTruthy();
                ++index;
            }
    ...
    describe('tag', () => {
        ...
        test('renders', () => {
            ...
            expect(getByTestId(`${defaultProps.testIdPrefix}::Title`).props.children).toEqual('tags');

            let index = 0;
            for (const tag of defaultProps.items) {
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::Type`).props.children).toEqual('tag');
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::Item`).props.children).toEqual(JSON.stringify(tag));
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::OnEdit`)).toBeTruthy();
                expect(getByTestId(`${defaultProps.testIdPrefix}::SettingsItemCard::${index}::OnDelete`)).toBeTruthy();
                ++index;
            }
